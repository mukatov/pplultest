...
import { User, Dumbbell, Trophy, Calendar, TrendingUp, Download, LogOut } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const { exercises, workoutSets, personalRecords, splits, activeSplitId } = useWorkoutStore();
  const t = useT();
  ...
  const last30 = Array.from({ length: 30 }, (_, i) => {
    ...
    const vol = userSets
      .filter(ws => ws.date.startsWith(dayStr))
      .reduce((sum, ws) => sum + ws.sets.reduce((s, set) => s + set.weight * set.reps, 0), 0);
    return { date: `${d.getMonth() + 1}/${d.getDate()}`, volume: vol };
  }).filter(d => d.volume > 0);
  ...
  const handleExport = () => {
    ...
    for (const ws of sorted) {
      ...
      ws.sets.forEach((s, i) => {
        rows.push([date, day?.label ?? ws.dayType, exercise?.name ?? exerciseId,
          String(i + 1), String(s.weight), String(s.reps), String(s.weight * s.reps)]);
      });
    }
    ...
    <div className="min-h-screen bg-[#171717] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 pb-28">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: t.totalSessions, value: userSets.length, icon: Calendar   },
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={byDay} barSize={28}>
                  ...
                  <Bar dataKey="sessions" radius={[4, 4, 0, 0]}>
                    {byDay.map((entry, index) => <Cell key={index} fill={entry.color} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-48 flex items-center justify-center text-[#737373] text-sm">{t.logWorkoutsToSeeData}</div>
            )}
          </div>
            ...
            {last30.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={last30}>
                  ...
                  <Line type="monotone" dataKey="volume" stroke="#e5e5e5" strokeWidth={2}
                    dot={{ fill: '#e5e5e5', strokeWidth: 0, r: 3 }} activeDot={{ r: 5, fill: '#fafafa' }} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-48 flex items-center justify-center text-[#737373] text-sm">{t.logWorkoutsToSeeData}</div>
        ...
        <div className="bg-[#262626] rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              ...
              {exerciseOptions.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
            </select>
          </div>
          {progressData.length > 1 ? (
            ...
            <div className="h-48 flex items-center justify-center text-[#737373] text-sm">{t.need2Sessions}</div>
          ) : (
            <div className="h-48 flex items-center justify-center text-[#737373] text-sm">{t.selectExerciseToView}</div>
          )}
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-4 py-4 rounded-2xl bg-[#262626] text-red-400 font-medium text-sm active:bg-[#2e2e2e] transition-colors"
        >
